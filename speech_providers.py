...
class SpeechProviderFactory:
    ...
    def get_provider(provider_name: Optional[str] = None) -> Optional[SpeechProvider]:
        ...
        Args:
            provider_name: Name of provider ('openai', 'local', 'ffmpeg', or None for auto)
        ...
        # FFmpeg+Whisper.cpp first (fastest), then OpenAI (most accurate), then local (fallback)
        providers = [FFmpegWhisperProvider(), OpenAIProvider(), LocalWhisperProvider()]
